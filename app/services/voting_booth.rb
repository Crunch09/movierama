...
class VotingBooth
  ...
  def vote(like_or_hate)
    ...
    set.add(@user)
    _update_counts
    _update_counts_for_digest_mail(like_or_hate)
    self
  end
  ...
  def _update_counts_for_digest_mail(like_or_hate)
    VoteCounter.create(user_id: @user.id, movie_id: @movie.id, type: like_or_hate)
  end
end
